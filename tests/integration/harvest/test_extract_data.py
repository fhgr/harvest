                ...
                if element not in ignored_element:
                    if element == 'post_text':
                        assert fuzz.ratio(gold_annotation[element]['surface_form'],
                                          response[index][element]) > ratio
                    else:
                        assert gold_annotation[element]['surface_form'] == response[index][element]
...
def load_test_data():
    def _load_test_data(file_name):
        file_path = os.path.join(os.path.dirname(__file__), '../../../corpus/goldDocuments', file_name)
...
def test_forum_angelman(load_test_data, compare):
...
def test_forum_bpdfamily(load_test_data, compare):
...
def test_forum_bitdefender(load_test_data, compare):
...
def test_forum_kaspersky(load_test_data, compare):
    forum_test_data = load_test_data(
...
def test_forum_community_scope(load_test_data, compare):
    forum_test_data = load_test_data("community.scope.org.uk.discussion.68941.disabled-mum.json")
    response = extract_data(forum_test_data['html'], forum_test_data['url'])['posts']
    compare(forum_test_data['gold_standard_annotation'], response, ['post_link'])


def test_forum_digitalfernsehen(load_test_data, compare):
...
def test_forum_ebaumsworld(load_test_data, compare):
    forum_test_data = load_test_data("forum.ebaumsworld.com.viewtopic.php.42095.json")
    response = extract_data(forum_test_data['html'], forum_test_data['url'])['posts']
    compare(forum_test_data['gold_standard_annotation'], response, ['datetime'])


def test_forum_glamour(load_test_data, compare):
...
def test_forum_futura_sciences(load_test_data, compare, remove_index):
...
def test_forum_sherdog(load_test_data, compare):
    forum_test_data = load_test_data("forums.sherdog.com.threads.all-time-goat-poll.3916359..json")
    response = extract_data(forum_test_data['html'], forum_test_data['url'])['posts']
    compare(forum_test_data['gold_standard_annotation'], response, ['datetime', 'post_link'])


def test_forum_kiwifarms(load_test_data, compare):
    forum_test_data = load_test_data(
...
def test_forum_myparkinsons(load_test_data, compare, remove_index):
...
def test_forum_skyscraperpage(load_test_data, compare):
    forum_test_data = load_test_data("skyscraperpage.com.forum.showthread.php.json")
    response = extract_data(forum_test_data['html'], forum_test_data['url'])['posts']
    compare(forum_test_data['gold_standard_annotation'], response, ['datetime', 'post_link'])


def test_forum_collegeconfidential(load_test_data, compare):
    forum_test_data = load_test_data(
...
def test_forum_airliners(load_test_data, compare):
...
def test_forum_amsel(load_test_data, compare):
    forum_test_data = load_test_data("www.amsel.de.multiple-sklerose-forum..json")
    response = extract_data(forum_test_data['html'], forum_test_data['url'])['posts']
    compare(forum_test_data['gold_standard_annotation'], response, ['datetime'])


def test_forum_android_hilfe(load_test_data, compare):
    forum_test_data = load_test_data(
...
def test_forum_computerbase(load_test_data, compare):
    forum_test_data = load_test_data(
...
def test_forum_drwindows(load_test_data, compare):
    forum_test_data = load_test_data(
...
def test_forum_fanfiction(load_test_data, compare):
    forum_test_data = load_test_data(
...
def test_forum_med1(load_test_data, compare):
    forum_test_data = load_test_data(
...
def test_forum_musiker_board(load_test_data, compare):
    forum_test_data = load_test_data(
